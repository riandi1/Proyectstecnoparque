...
<div class="mb-3">
    <div class="float-right">
        @can('create', new App\Models\Vehicle)
            <a href="{{ route('vehicles.create') }}" class="btn btn-success">{{ __('vehicle.create') }}</a>
        @endcan
    </div>
            ...
            <table class="table table-sm table-responsive-sm table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{!! $vehicle->name_link !!}</td>
                        <td>{{ $vehicle->description }}</td>
                        <td class="text-center">
                            @can('view', $vehicle)
                                <a href="{{ route('vehicles.show', $vehicle) }}" id="show-vehicle-{{ $vehicle->id }}">{{ __('app.show') }}</a>
                            @endcan
                        </td>
                    </tr>
                    @endforeach
